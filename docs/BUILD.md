...
- A separately built or installed `LIB_BaComputerGames.dll`, API 1.0.1 or newer, compiled for the game's Mono profile, with its matching 1.0.1+ `ModManifest.asset` alongside the DLL (or supplied through `-McgManifest`).
...
The build rejects a preview MCG dependency. Manifest/catalog versions must be 1.0.1, and the resulting assembly and file versions must be 1.0.1.0. Its ignored `build-status.json` records dependency versions and hashes without private paths.
